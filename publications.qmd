...
pubs <- pub_data$publications |>
  ...
  mutate(
    ...
    sotl_label = if_else(is_sotl == TRUE, "SoTL", "Discipline-specific"),
    school_display = replace_na(network_schools, "—")
  )
```
...
pubs |>
  ...
  datatable(
    escape = FALSE,  # This is critical for HTML links to render
    filter = "top",
    options = list(
      pageLength = 25,
...
ggplot(year_counts, aes(x = year, y = n)) +
  ...
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5, size = 14, face = "bold", colour = uofg_blue),
